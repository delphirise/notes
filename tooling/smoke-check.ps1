...
$repoRoot = Split-Path -Parent $toolingRoot
$templatesPath = Join-Path $repoRoot 'app\pages\Templates.html'
$scriptPath = Join-Path $repoRoot 'app\src\templates.js'
$fixturesPath = Join-Path $toolingRoot 'tests\note-fixtures.json'

if (-not (Test-Path $templatesPath)) {
    Write-Host "[FAIL] Missing app/pages/Templates.html at $templatesPath" -ForegroundColor Red
    exit 1
}

if (-not (Test-Path $scriptPath)) {
    Write-Host "[FAIL] Missing app/src/templates.js at $scriptPath" -ForegroundColor Red
    exit 1
}

if (-not (Test-Path $fixturesPath)) {
    Write-Host "[FAIL] Missing tooling/tests/note-fixtures.json at $fixturesPath" -ForegroundColor Red
    exit 1
}
